...
async def login_for_access_token(db:Session = Depends(get_db),form_data: OAuth2PasswordRequestForm = Depends()):
    user = CRUD.authenticate_user(db,form_data.client_id, form_data.username, form_data.password)
    if not user:
        raise HTTPException(
...
async def make_purchase(purchase: schemas.purchaseCreate,current_user: models.User = Depends(CRUD.get_current_active_user),db:Session=Depends(get_db)):
    ...
    if current_user.is_worker==0:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Workers can not make purchases!")